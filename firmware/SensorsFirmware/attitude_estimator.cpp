...
// Constructor
AttitudeEstimator::AttitudeEstimator(boolean quickLearn)
{
...
// Reset functions
void AttitudeEstimator::reset(boolean quickLearn, boolean resetGyroBias) // Note: Resets the entire class, except for the magnetometer calibration, the acc-only resolution method, and the configuration variables (i.e. the PI gains and the quick learn time)
{
...
void AttitudeEstimator::resetAll(boolean quickLearn) // Note: Resets absolutely everything about the class
{
...
void AttitudeEstimator::resetState(boolean resetGyroBias) // Note: Currently used by reset() only
{
	// Declare variables
	int i;
...
void AttitudeEstimator::setAttitudeFused(double yaw, double pitch, double roll, boolean hemi)
{
...
* optimised to use the least number of doubleing point operations as possible. The source code of this and the subordinate functions thereof
...
void AttitudeEstimator::updateQy(double accX, double accY, double accZ, double magX, double magY, double magZ)
{
	...
	// of doubleing point arithmetic) are pretty much irrelevant. This is firstly because they will only
